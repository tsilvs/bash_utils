...
adb.apps.diff() {
	...
	[[ ! "$u1" =~ ^[0-9]+$ ]] && {
		echo "User 1 ID must be a number"
		return 1
	}
	[[ ! "$u2" =~ ^[0-9]+$ ]] && {
		echo "User 2 ID must be a number"
		return 1
	}
...
adb.app.install() {
	...
	[[ ${#apps[@]} -eq 0 ]] && {
		echo "At least one package required"
		return 1
	}
	...
	for u in "${users[@]}"; do
		[[ ! "$u" =~ ^[0-9]+$ ]] && {
			echo "User ID must be a number: $u"
			return 1
		}
		for app in "${apps[@]}"; do
...
register_simple_completion "adb.users"
register_simple_completion "adb.app.ls" "-u" "--user"
register_simple_completion "adb.apps.diff"
register_simple_completion "adb.app.install"
register_simple_completion "adb.app.uninstall"
register_simple_completion "adb.app.in"
register_simple_completion "adb.app.un"
